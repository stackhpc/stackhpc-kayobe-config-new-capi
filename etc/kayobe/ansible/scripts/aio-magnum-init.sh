...
if ! $KOLLA_OPENSTACK_COMMAND image list | grep -q ubuntu-2004-kube-v1.26.0; then
  ...
  curl -L --fail -o ${IMAGE_PATH}ubuntu-2004-kube-v1.26.0.qcow2 https://minio.services.osism.tech/openstack-k8s-capi-images/ubuntu-2004-kube-v1.26/ubuntu-2004-kube-v1.26.0.qcow2
  $KOLLA_OPENSTACK_COMMAND image create ubuntu-2004-kube-v1.26.0 \
    --file ${IMAGE_PATH}/ubuntu-2004-kube-v1.26.0.qcow2 \
    --disk-format qcow2 \
    --container-format bare \
    --public
  $KOLLA_OPENSTACK_COMMAND image set ubuntu-2004-kube-v1.26.0 --os-distro ubuntu --os-version 20.04
  
  echo Registering new driver template..

  $KOLLA_OPENSTACK_COMMAND coe cluster template create new_driver \
  --coe kubernetes \
  --image ubuntu-2004-kube-v1.26.0 \
  --external-network public1 \
  --label kube_tag=v1.26.0 \
